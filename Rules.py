                ...
                while isIn and count > 0:
                    ...
                    try:
                        ...
                        msg +=('\t...'\
                              ...
                              +'**'+ Data[server.id]['RuleList'][rule][index:index+len(query)]\
                              +'**'+ Data[server.id]['RuleList'][rule][index+len(query):boundUpper]\
                              +'...').replace('\n','  ')+'\n\n'
                    except ValueError:
                        isIn = 0
                    count -= 1
...
async def f(Data, channels, server, payload, *text):
	await find(Data, channels, server, payload, *text)

async def search(Data, channels, server, payload, *text):
	await find(Data, channels, server, payload, *text)
        ...
        for rule in ruletxt:
            try:
                ...
                with urllib.request.urlopen('https://raw.githubusercontent.com/dmouscher/nomic/master/Game_4/rules/txt/' + str(rulenum) + '.txt') as ruleresponse:
                    ...
                    Data[server.id]['RuleList'][rulenum] = ruleresponse.read().decode("utf-8")#[1].replace('&nbsp;', ' ').replace('\\n', '\n')
            except:
                print('ERROR')
                print(rule)
